...
on your system) by running the ``crontab -e`` command::
      ...
      */5 * * * * /usr/local/bro/bin/zeekctl cron
...
your own Zeek script ``mypolicy.zeek`` in the ``share/bro/site`` directory,
...
*update* *[<nodes>]*
    ...
    After a change to Zeek policy scripts, this command updates the Zeek
    processes on the given nodes *while they are running* (i.e., without
    requiring a restart_). However, such dynamic
    updates work only for a *subset* of Zeek's full configuration. The
...
*PluginZeekDir* (string, default "$\{ZeekBase}/lib/bro/plugins")
