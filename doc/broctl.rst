...
Running *BroControl* requires the following prerequisites:
  ...
  - In a cluster setup, *rsync* and *ssh* must be installed, and *sshd*
    must be installed and running on every host (except the manager) in
    the cluster.
...
With all prerequisites in place, perform the following steps (on the manager
host) to install a Bro cluster (as the Bro user):
