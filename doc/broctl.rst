...
.. class:: opening
    ...
    Control* has two modes of operation: a *stand-alone* mode for
    ...
    for both types; just keep in mind that if this documents refers to
    "nodes" and you're in a stand-alone setup, there's is only a
...
once a day. Logs files of workers and proxies are discarded at the
...
*process*
    ...
    be given, the latter must be prefixed by ``--``.
...
*scripts* *[-c] [<nodes>]*
    ...
    will be parsed by the node at startup.
...
*PolicyDirBroCtl* (string, default "$\{SpoolDir}/policy/broctl")
    Directory for additioal broctl policy scripts.

.. _PolicyDirSiteInstall:

*PolicyDirSiteInstall* (string, default "$\{SpoolDir}/policy/site")
    Directory where the shell copies local policy scripts when installing.

.. _PolicyDirSiteInstallAuto:

*PolicyDirSiteInstallAuto* (string, default "$\{SpoolDir}/policy/auto")
