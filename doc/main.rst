...
BroControl's git repository is located at https://github.com/zeek/broctl
...
are located in ``share/bro/site``.  The file called ``local.zeek`` gets
...
your own Bro script ``mypolicy.zeek`` in the ``share/bro/site`` directory,
...
When Bro starts up, the first script it loads is init-bare.zeek, followed
...
The next scripts loaded are ``local-networks.zeek`` and ``broctl-config.zeek``.
...
inserted into the ``broctl-config.zeek`` script.
