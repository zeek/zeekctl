...
try:
    ...
    # Tested with 11.0 and it seems to work good enough.
...
class WebSocketClient:
    ...
    def recv_json(self, timeout=None):
        ...
        return json.loads(self.__c.recv(timeout=timeout))
    ...
    def v1_recv_event(self, timeout=None):
        """
        ...
        try:
            d = self.recv_json(timeout=timeout)
        except (
            TimeoutError,
            websockets_exceptions.ConnectionClosed,
            websockets_exceptions.ConnectionClosedError,
...
def ws_send_events(events, topic):
    ...
    with ws:
        ...
        for node, event, args, result_event in events:
            # XXX: This is dumb. ZeroMQ uses dot separated topics while broker uses slash.
            #      Probably ZeroMQ should've used slash, too, but NATS treats dots special.
            #      That's where this originates from. Or we need some "topic_separator"
            #      constant somewhere. Meh.
            #
            #      Also, the response always comes in on /zeek/control/<command>/<node_name>
            #      with slashees as that's hard-coded in the frameworks/control scripts.
            #
            #      Grrrr. But it works and the better fix is probably to switch to a
            #      unique reply topic instead.
            backend = config.Config.clusterbackend.lower()
            if backend == "zeromq":
                topic = "zeek.cluster.node." + node.name + "."
            elif backend == "broker":
                topic = "zeek/cluster/node/" + node.name + "/"
            else:
                raise NotImplementedError(config.Config.clusterbackend)
            ...
            try:
                ...
                if result_event != rname:
                    results += [
                        (node, False, "expected '{result_event}' got '{rname}'")
                    ]
                    continue

                # Figure out which node sent the reply. It's the last part
                # in the / separated reply topic.
