...
def _make_zeek_params(node, live):
    ...
    # 0) The cluster backend specific script (e.g Broker, ZeroMQ) if not
    #    running in standalone mode. This must come early for SitePolicyScripts
    #    to redef certain settings.
    ...
    if not node_mod.is_standalone(node):
        args += ["policy/frameworks/cluster/backend/" + config.Config.clusterbackend.lower()]
