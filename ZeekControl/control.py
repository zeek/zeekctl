...
def _make_zeek_params(node, live):
    ...
    # 1) Policy scripts for the selected cluster backend. This is needed
    #    even in standalone mode for print or netstats command which are
    #    using cluster functionality under the hood.
    # 2) SitePolicyScripts (local.zeek by default) gives a common set of loaded
    #    scripts for all nodes.
    # 3) The common configuration of zeekctl is loaded via the zeekctl package.
    # 4) The distribution's default settings for node configuration are loaded
    ...
    #    a notice policy).
    # 5) Autogenerated zeekctl scripts are loaded, which may contain
    #    settings that override the previously loaded scripts.
    #    (e.g. see Log::default_rotation_interval)
    args += [
        "policy/frameworks/cluster/backend/" + config.Config.clusterbackend.lower()
    ]
