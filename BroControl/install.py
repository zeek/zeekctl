...
def install(local_only):
    ...
    makeCerts(config.Config.policydirsiteinstallauto)
...
"""
    ...
    def hexify(fn):
        try:
            ...
            util.error("cannot read certificate file: %s" % err)

        return ''.join("\\x" + x.encode('hex').upper() for x in s)

    def writecerts(s=""):
        certfile = os.path.join(config.Config.policydirsiteinstallauto, "certs.bro")
        with open(certfile, 'w') as f:
            f.write("# Don't edit!  This file is automatically generated.\n")
            f.write(s)


    cert_dir = config.Config.certdir

    # If the directory doesn't exist, then just create an empty file.
    if not os.path.isdir(cert_dir):
        writecerts()
        return

    # Gather a list of files in cert_dir with filename ending in ".der" (case
    # insensitive).
    try:
        certs = [fn for fn in os.listdir(cert_dir) if fn.lower().endswith(".der") and os.path.isfile(os.path.join(cert_dir, fn))]
    except OSError, err:
        util.error("cannot read certificate directory: %s" % err)

    # If no files were found, then just create an empty file.
    if not certs:
        writecerts()
        return

    util.output("converting certs ...", False)
    ...
    for cert in certs:
        ...
        # We don't really care about the name for now, so just use the filename
        # as the subject.
        subject = os.path.basename(full)

        s += BRO_CERT_TEMPLATE % dict(subject=subject, cert=cert)
