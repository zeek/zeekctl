...
def _makeBroParams(node, live, add_manager=False):
    ...
    # Order here is important.  First, the common configuration of broctl
    # is loaded via the broctl package.  Second the distribution's default
    # settings for the node configuration are loaded from either the
    # cluster framework or broctl standalone scripts.  The cluster
    # framework case will take care of overriding default node
    # based on any user specified site/local-<node>.bro scripts.
    # Third, autogenerated broctl scripts are loaded, which may contain
    # settings that override the previously loaded node-specific scripts.
    # Finally, the site/local.bro is loaded and can override any setting.
    args += ["broctl"]
    if node.type == "standalone":
